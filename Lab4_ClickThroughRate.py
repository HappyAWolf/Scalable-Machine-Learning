# Databricks notebook source exported at Fri, 14 Aug 2015 13:00:58 UTC
# MAGIC %md
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return inputData.flatMap(lambda a: a).distinct().zipWithIndex().collectAsMap()
...
def parsePoint(point):
    ...
    """
    parsed = point.split(',')[1:]
    result = []
    for j in range(len(parsed)):
      result.append((j, parsed[j]))
    return result
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    values = []
    for ind in rawFeats:
      if ind in indices or ind not in ctrOHEDict:
        continue
      else:
        indices.append(OHEDict[ind])
        values.append(1.)
...
# TODO: Replace <FILL IN> with appropriate code
model0 = LogisticRegressionWithSGD.train(data = OHETrainData, iterations = numIters, regParam = regParam, step = stepSize, regType = regType, intercept = includeIntercept)
sortedWeights = sorted(model0.weights)
print sortedWeights[:5], model0.intercept
...
def evaluateResults(model, data):
    ...
    Returns:
        float: Log loss for the data.
    """
    return data.map(lambda a: (a.label, getP(a.features, model.weights, model.intercept))).map(lambda a: computeLogLoss(a[1], a[0])).mean()
...
# TODO: Replace <FILL IN> with appropriate code
logLossValBase = OHEValidationData.map(lambda a: computeLogLoss(classOneFracTrain, a.label)).mean()
...
def parseHashPoint(point, numBuckets):
    ...
    Returns:
        LabeledPoint: A LabeledPoint with a label (0.0 or 1.0) and a SparseVector of hashed
            features.
    """
    hashed = hashFunction(numBuckets, parsePoint(point), False)
    return LabeledPoint(point.split(',')[0], SparseVector(numBuckets, hashed))
    

numBucketsCTR = 2 ** 15
hashTrainData = rawTrainData.map(lambda a: parseHashPoint(a, numBucketsCTR))
hashTrainData.cache()
hashValidationData = rawValidationData.map(lambda a: parseHashPoint(a, numBucketsCTR))
hashValidationData.cache()
hashTestData = rawTestData.map(lambda a: parseHashPoint(a, numBucketsCTR))
hashTestData.cache()
...
def computeSparsity(data, d, n):
    ...
    """
    return data.map(lambda a: len(a.features.indices)*1.0).mean()/d
