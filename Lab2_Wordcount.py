# Databricks notebook source exported at Fri, 14 Aug 2015 12:59:06 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda a: (a[0], sum(a[1])))
...
def removePunctuation(text):
    ...
    s = text.strip().lower()
    punc = "!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~"
    return ''.join(ch for ch in s if ch not in punc).strip()
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.filter(lambda a: len(a.strip()) > 0)
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15, lambda s: -s[1])
