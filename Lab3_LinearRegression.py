# Databricks notebook source exported at Fri, 14 Aug 2015 12:59:55 UTC
# MAGIC %md
...
def parsePoint(line):
    ...
    a, b = line.split(',', 1)
    label = float(a)
    feature = [float(j) for j in b.split(',')]
...
maxYear = onlyLabels.takeOrdered(1, lambda a: -a)[0]
...
averageTrainYear = (parsedTrainData
                    .map(lambda a: a.label).reduce(lambda a, b: a+b)/float(parsedTrainData.count()))
...
def calcRMSE(labelsAndPreds):
    ...
    """
    import math
    return math.sqrt(labelsAndPreds.map(lambda a: squaredError(a[0], a[1])).reduce(lambda a, b: a+b) / labelsAndPreds.count())
...
#averageValYear = parsedValData.map(lambda a: a.label).reduce(lambda a, b: a+b)/float(parsedValData.count())
labelsAndPredsVal = parsedValData.map(lambda a: (a.label, averageTrainYear))
rmseValBase = calcRMSE(labelsAndPredsVal)

#averageTestYear = parsedTestData.map(lambda a: a.label).reduce(lambda a, b: a+b)/float(parsedTestData.count())
...
def twoWayInteractions(lp):
    ...
    for i in range(len(features)):
        for j in range(len(features)):
            two_way_features.append(features[i]*features[j])
    #two_way_features = itertools.product(features, features)
    new_features = np.hstack([features, two_way_features])
